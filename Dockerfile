...
RUN set -eux; \
    ...
    echo "Applying FIPS compliance modifications to golang-fips/go..."; \
    ...
    # \
    # CRITICAL FIX: Remove TLS 1.3 ChaCha20-Poly1305 cipher suite (non-FIPS algorithm)
    # Client Feedback (@mattia-moffa): Patch file approach was problematic - applied to golang-fips/go
    # after golang-fips patches modified cipher_suites.go, causing context mismatch and silent failures.
    # Solution: Use sed to directly remove ALL ChaCha20 references from crypto/tls.
    ...
    echo "FIPS modifications applied successfully"; \
    cd src; \
    CGO_ENABLED=1 \
